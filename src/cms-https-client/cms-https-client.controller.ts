...
export class CmsHttpsClientController {
     ...
     * and the host ID is taken from the URL parameters. The request body, which should
     ...
     * 호스트 ID는 URL 파라미터에서 가져옵니다. 토큰을 포함하지 않아야 하는 요청 본문은
     * 처리를 위해 CmsHttpsClientService로 전달됩니다.
     *
     * @param req - The Express request object, containing user information from the JWT.
     * @param hostId - The unique identifier of the host to which the request will be forwarded.
     ...
     * @param hostId - 요청이 전달될 호스트의 고유 식별자.
     * @param request - CmsForwardRequestWithoutToken에 따라 클라이언트로부터의 요청 본문.
     * @returns CMS API의 응답으로 해결되는 Promise.
     */
    @Post('forward/:hostId')
    async forwardRequest(@Req() req, @Param('hostId') hostId: string, @Body() request: CmsForwardRequestWithoutToken) {
