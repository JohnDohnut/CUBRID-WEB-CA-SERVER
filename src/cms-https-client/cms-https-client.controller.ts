...
import { CmsForwardClientRequest } from '@type/index';
...
export class CmsHttpsClientController {
     ...
     * contain hostUid and task, is then passed to the CmsHttpsClientService for processing.
     *
     * 클라이언트로부터의 인증된 요청을 CMS API로 전달합니다.
     * 이 엔드포인트는 JWT 인증을 필요로 합니다. 사용자 ID는 JWT에서 추출되며,
     * 호스트 ID는 요청 본문에서 가져옵니다. hostUid와 task를 포함하는 요청 본문은
     * 처리를 위해 CmsHttpsClientService로 전달됩니다.
     *
     * @param req - The Express request object, containing user information from the JWT.
     * @param request - The request body from the client, conforming to CmsForwardClientRequest.
     * @returns A Promise that resolves with the response from the CMS API.
     *
     * @param req - JWT에서 사용자 정보를 포함하는 Express 요청 객체.
     * @param request - CmsForwardClientRequest에 따라 클라이언트로부터의 요청 본문 (hostUid, task 포함).
     * @returns CMS API의 응답으로 해결되는 Promise.
     */
    @Post('forward')
    async forwardRequest(@Req() req, @Body() request: CmsForwardClientRequest) {
